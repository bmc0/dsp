...
#define S24_SIGN_EXTEND(x) ((x & 0x800000) ? x | ~0x7fffff : x)
	...
	#define SAMPLE_TO_U8(x)     ((unsigned char) ((x * 128.0 + 127.0 <= -0.5) ? 0 : lround(x * 128.0 + 127.0)))
	#define U8_TO_SAMPLE(x)     (((sample_t) x - 127.0) / 128.0)
	#define SAMPLE_TO_S8(x)     ((signed char) ((x * 128.0 >= 127.5) ? 127 : lround(x * 128.0)))
	#define S8_TO_SAMPLE(x)     ((sample_t) x / 128.0)
	#define SAMPLE_TO_S16(x)    ((signed short) ((x * 32768.0 >= 32767.5) ? 32767 : lround(x * 32768.0)))
	#define S16_TO_SAMPLE(x)    ((sample_t) x / 32768.0)
	#define SAMPLE_TO_S24(x)    ((signed int) ((x * 8388608.0 >= 8388607.5) ? 8388607 : lround(x * 8388608.0)))
	#define S24_TO_SAMPLE(x)    ((sample_t) S24_SIGN_EXTEND(x) / 8388608.0)
	#define SAMPLE_TO_S32(x)    ((signed int) ((x * 2147483648.0 >= 2147483647.5) ? 2147483647 : lround(x * 2147483648.0)))
	#define S32_TO_SAMPLE(x)    ((sample_t) x / 2147483648.0)
#else
	/* these do not clip, but are also not bit perfect */
	#define SAMPLE_TO_U8(x)     ((unsigned char) lround(x * 127.0 + 127.0))
	#define U8_TO_SAMPLE(x)     (((sample_t) x - 127.0) / 128.0)
	#define SAMPLE_TO_S8(x)     ((signed char) lround(x * 127.0))
	#define S8_TO_SAMPLE(x)     ((sample_t) x / 128.0)
	#define SAMPLE_TO_S16(x)    ((signed short) lround(x * 32767.0))
	#define S16_TO_SAMPLE(x)    ((sample_t) x / 32768.0)
	#define SAMPLE_TO_S24(x)    ((signed int) lround(x * 8388607.0))
	#define S24_TO_SAMPLE(x)    ((sample_t) S24_SIGN_EXTEND(x) / 8388608.0)
	#define SAMPLE_TO_S32(x)    ((signed int) lround(x * 2147483647.0))
	#define S32_TO_SAMPLE(x)    ((sample_t) x / 2147483648.0)
#endif
