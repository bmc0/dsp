...
struct effect * ladspa_host_effect_init(struct effect_info *ei, struct stream_info *istream, char *channel_selector, const char *dir, int argc, char **argv)
{
	...
	if (lt_dlinit() || lt_dlsetsearchpath(search_path)) {
		LOG(LL_ERROR, "dsp: %s: error: failed to initialize libltdl: %s\n", argv[0], lt_dlerror());
		return NULL;
	}
	...
	if ((state->lt_handle = lt_dlopenext(path)) == NULL) {
		LOG(LL_ERROR, "dsp: %s: error: failed to open LADSPA plugin: %s: %s\n", argv[0], path, lt_dlerror());
		goto fail;
	}

	/* Get address of ladspa_descriptor() */
	if ((descriptor_fn = lt_dlsym(state->lt_handle, "ladspa_descriptor")) == NULL) {
		LOG(LL_ERROR, "dsp: %s: %s: error: could not find ladspa_descriptor()\n", argv[0], path);
		goto fail;
	}
	...
	if (state->desc == NULL) {
		LOG(LL_ERROR, "dsp: %s: %s: error: could not find plugin: %s\n", argv[0], path, argv[2]);
		goto fail;
	}
	desc = state->desc;
	...
	for (unsigned long i = 0; i < desc->PortCount; ++i) {
		...
		if (LADSPA_IS_PORT_INPUT(pd) && LADSPA_IS_PORT_OUTPUT(pd)) {
			LOG(LL_ERROR, "dsp: %s: %s: %s: error: port '%s' (%lu) is both an input and an output\n", argv[0], path, argv[2], desc->PortNames[i], i);
			goto fail;
		}
		if (LADSPA_IS_PORT_AUDIO(pd) && LADSPA_IS_PORT_CONTROL(pd)) {
			LOG(LL_ERROR, "dsp: %s: %s: %s: error: port '%s' (%lu) is both audio and control\n", argv[0], path, argv[2], desc->PortNames[i], i);
			goto fail;
		}
	...
	if (state->n_in <= 1)
		...
		LOG(LL_ERROR, "dsp: %s: %s: %s: error: expected %d input channels, got %d\n", argv[0], path, argv[2], state->n_in, selected_channel_count);
		goto fail;
	}
	if (state->n_out < 1) {
		LOG(LL_ERROR, "dsp: %s: %s: %s: error: plugin has no outputs\n", argv[0], path, argv[2]);
		goto fail;
	}
	...
	for (int i = 0; i < state->n_handles; ++i) {
		if ((state->handles[i] = desc->instantiate(desc, istream->fs)) == NULL) {
			LOG(LL_ERROR, "dsp: %s: %s: %s: error: instantiate() failed\n", argv[0], path, argv[2]);
			goto fail;
		}
